...
class CoroutineDumpPanel(project: Project, consoleView: ConsoleView, toolbarActions: DefaultActionGroup, val dump: List<CoroutineState>) :
    ...
    init {
        ...
        coroutinesList.cellRenderer = CoroutineListCellRenderer()
        coroutinesList.selectionMode = ListSelectionModel.SINGLE_SELECTION
        coroutinesList.addListSelectionListener {
...
//        toolbarActions.add(SortThreadsAction())
        ...
        leftPanel.add(ScrollPaneFactory.createScrollPane(coroutinesList, SideBorder.LEFT or SideBorder.RIGHT), BorderLayout.CENTER)

        val splitter = Splitter(false, 0.3f)
        splitter.firstComponent = leftPanel
        splitter.secondComponent = consoleView.component
        ...
        val editor = CommonDataKeys.EDITOR.getData(DataManager.getInstance().getDataContext(consoleView.preferredFocusableComponent))
    ...
    private inner class CoroutineListCellRenderer : ColoredListCellRenderer<Any>() {
        ...
        override fun customizeCellRenderer(list: JList<*>, value: Any, index: Int, selected: Boolean, hasFocus: Boolean) {
            ...
            var detail: String? = state.coroutineStateDetail
            if (detail == null) {
                detail = state.state
            }
            if (detail.length > 30) {
    ...
    private inner class MergeStacktracesAction :
        ToggleAction("Merge Identical Stacktraces", "Group coroutines with identical stacktraces", AllIcons.General.CollapseAll),
        DumbAware {
    ...
    private class CopyToClipboardAction(private val myCoroutinesDump: List<CoroutineState>, private val myProject: Project) :
        ...
        override fun actionPerformed(e: AnActionEvent) {
            ...
            GROUP.createNotification("Full coroutine dump was successfully copied to clipboard", MessageType.INFO).notify(myProject)
        }
